...
> **Good to know:** This is automatically called for you when using the Pilot.js `AppEntry.js` file. [Learn more](/docs/managed-entry).
...
For native apps, make sure you're using [Web Props](/docs/web-props) to take advantage of this feature. Server-only environment variables are not available if you're using `getServerSideProps` or `getStaticProps` on the native runtime.
